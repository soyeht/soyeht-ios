...
default-off infrastructure. The remaining work is the explicit enforcement flip
...
- `OwnerApprovalEnforcementPolicy::default()` is `LegacyOnly` for every
  operation.
- `OwnerEventsRouterState::new(...)` installs that default policy.
- The daemon router construction in `household_bootstrap.rs` does not override
  the owner-approval policy for production.
...
If B/defer is chosen, a flip PR still needs an explicit production control. It
must not be an accidental constructor change.

- Add a named rollout/config mechanism that turns on the approved
  ...
  recovery-code policy switch while preserving the reviewed break-glass
  semantics, not an active-count gate.
- The rollout mechanism needs an explicit rollback lever: operators must be
  able to return the policy to `LegacyOnly` cleanly, with the legacy/v1 path
  preserved for active users, if the flip shows a production issue.
