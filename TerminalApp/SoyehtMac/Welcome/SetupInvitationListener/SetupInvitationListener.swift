...
///
/// On first launch, before showing HouseNamingView, finds the iPhone's setup
/// invitation endpoint, claims the token, and notifies the iPhone with the Mac
/// engine URL it should use for bootstrap naming.
final class SetupInvitationListener: @unchecked Sendable {
    ...
    /// Finds and claims a setup invitation. The direct path is the only complete
    /// ceremony because it also notifies the iPhone after the engine accepts the
    /// claim; a claim without notify leaves the iPhone waiting forever.
    func listen() async -> Outcome {
        await listenViaTailscalePeerProbe()
    }
