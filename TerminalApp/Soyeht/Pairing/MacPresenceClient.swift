...
final class MacPresenceClient: NSObject, ObservableObject {
    ...
    func connect() {
        ...
        guard let url = MacLocalWebSocketEndpoint.presenceURL(
            host: host,
            presencePort: endpoint.presencePort,
            macID: macID
        ) else {
    ...
    private func handleText(_ text: String) {
        ...
        do {
            ...
            // Every presence-client text frame is supposed to be JSON.
            presenceClientLogger.error("client_decode_failed error=\(error.localizedDescription, privacy: .public)")
            return
        }
        guard let json = parsed as? [String: Any], let type = json["type"] as? String else {
            presenceClientLogger.error("client_envelope_invalid")
            return
        }
...
enum MacLocalWebSocketEndpoint {
    static func presenceURL(host: String, presencePort: Int, macID: UUID) -> URL? {
        EndpointPolicy.macLocalPresenceWebSocketURL(
            host: host,
    ...
    static func paneAttachURL(host: String, port: Int, paneID: String, nonce: String) -> URL? {
        EndpointPolicy.macLocalPaneAttachWebSocketURL(
            host: host,
            attachPort: port,
            paneID: paneID,
            nonce: nonce
