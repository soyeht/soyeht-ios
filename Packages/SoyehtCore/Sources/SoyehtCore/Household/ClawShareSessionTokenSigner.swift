...
/// endpoint, expires_at)` and is signed by the guest device key — so a
/// stolen credential blob alone cannot open a session.
///
...
/// `household_rs::claw_share_data_tunnel::SessionAuthToken`:
/// the signature covers the canonical CBOR of the unsigned body
/// `{session_id, credential_hash, endpoint, expires_at}` (canonical key
/// order is enforced by `HouseholdCBOR`). Pinned by
