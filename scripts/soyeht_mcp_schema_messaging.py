...
TOOLS_MESSAGING = [
    ...
    {
        "name": "message_agent",
        "description": "High-level durable tool for agent-to-agent communication in Soyeht. It never creates panes or starts processes. Each existing target returns its own delivered, queued, blocked, mcp_not_connected, agent_not_running, or not_live result; one unavailable target does not cancel the others. Messages are persisted only for eligible targets before delivery, and policy blocks are enforced. automatic uses semantic inbox only with an observed wake+read adapter; otherwise terminal delivery is deferred until no human draft is open. If a target has no connected MCP client, report that fact and ask the user whether they want to start or restart an agent there; never type a launch command without confirmation. inbox guarantees zero PTY writes. Use displayReference=[name] in prose and copy only the machine-routing UUID or legacy @handle from list_agents.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "conversationIDs": {"type": "array", "items": {"type": "string"}},
