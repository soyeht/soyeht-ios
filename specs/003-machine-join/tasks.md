...
- [X] T050 [US3] Ensure failed paths clear pending queue entries without retry but preserve the candidate's ability to regenerate a fresh QR (no client-side cache poisoning) in `/Users/macstudio/Documents/SwiftProjects/iSoyehtTerm/Packages/SoyehtCore/Sources/SoyehtCore/Household/JoinRequestQueue.swift` — Redesigned `JoinRequestQueue` with explicit `EntryState` (`pending` / `inFlight`) so the failure-path API is reachable in the real US3 pipeline (claim → biometric → sign → POST → fail). `claim` now transitions `pending → inFlight` without removing; `confirmClaim` finalizes success terminally; `failClaim` clears any state for terminal failures (`hhMismatch`, `certValidationFailed`, `signingFailed`, `serverError`, etc.); `revertClaim` returns inFlight → pending for the spec.md US3 #3 non-terminal cases (`biometricCancel`, `biometricLockout`) — card returns to pre-Confirm state and the request stays pending until TTL. `acknowledgeByMachine` and `pendingEntries(now:)` correctly handle both states. New `Event` variants `claimedInFlight` and `revertedToPending`. Idempotency keying unchanged — fresh nonce always re-enqueues. 33 tests in `JoinRequestQueueTests.swift` covering full lifecycle.
