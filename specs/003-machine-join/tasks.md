...
- [X] T011 [P] Add `OperatorFingerprint` determinism + cross-repo binding tests against the 16+-tuple fixture (per spec.md SC-004 floor) in `/Users/macstudio/Documents/SwiftProjects/iSoyehtTerm/Packages/SoyehtCore/Tests/SoyehtCoreTests/OperatorFingerprintTests.swift` — Determinism + bit-extraction self-vectors landed; cross-repo fixture binding closed 2026-05-07 with `crossRepoFingerprintBindingMatchesTheyos` consuming the 16-tuple T005 fixture (full SC-004 anchor). Asserts fixture shape (positional indices contiguous from 0, 6 words per vector, joined `fingerprint` matches space-joined words) outside the per-vector loop, then derives via `OperatorFingerprint.derive` and compares words byte-equal per vector. When theyos expands to the joint 100k cross-locale set the test will continue to consume whatever ships byte-identical.
