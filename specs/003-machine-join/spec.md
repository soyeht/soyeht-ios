...
4. **APNS payload is literally `b'{"v":1}'`** — bytes-identical every time. No hint field, no metadata, nothing. The iPhone's silent-push handler never reads the payload beyond confirming arrival; it always performs a Tailscale long-poll fetch on wake. This is the trade-off for full opacity from Apple's infrastructure.
...
- **FR-023**: The app MUST register its APNS device token with the household via a PoP-authenticated `POST /api/v1/household/apns-register` call (sender chosen by the household leader-election protocol §13; iPhone selects via snapshot.members ranking). The registration call MUST include only the device token, the iPhone's `p_id`, and a P-256 ECDSA signature over `(p_id, device_token, timestamp)`. It MUST NOT include `hh_id`, hostname, IP, or any household-scoped data — the household identifies the request from the PoP-signed `p_id`.
...
- **FR-025**: The app MUST deregister via `POST /api/v1/household/apns-deregister` when `HouseholdSession` is cleared (logout, household-leave, or session-corruption recovery). Deregistration MUST be best-effort with retry; failure does not block session clearing locally, but a follow-up retry MUST run on next foreground until success or 24 h, whichever first.
- **FR-026**: On every app foreground, if the long-poll connection establishes successfully and the household reports an unknown device token for `p_id`, the iPhone MUST re-register automatically. This recovers from app reinstall, restore-from-backup, or APNS-token-rotation-while-offline without user action.
