...
| ST-Q-MCPA-160 | A real recipient agent has unfinished user input when a real sender agent relays an MCP v2 message. | `QA/scripts/soyeht_agent_driven_e2e.py --typing-collision-only` against Soyeht Dev | A focused OpenCode pane accepts text without Enter; a real Codex sends a durable `deferredTerminal` message; its terminal-delivery timestamp stays empty and its token stays absent before Enter; after Enter it is delivered, OpenCode replies through MCP contract 2, and Codex observes the reply. Dynamic alternate-screen capture of the draft itself is informative but not used as the gate oracle. |
