...
| ST-Q-MCPA-106 | List all panes across all workspaces. | Direct MCP `list_panes` | Response includes `listedPanes` with `conversationID`, `workspaceID`, `handle`, `path`, and `declaredAgent` for every open pane. |
...
| ST-Q-MCPA-135 | Codex-created panes get the same even batch layout. | `create_worktree_panes` with `repo=<test repo>`, `agent="codex"`, `names=["qa-agent-codex-a","qa-agent-codex-b","qa-agent-codex-c","qa-agent-codex-d"]`, `newWorkspace=true`, `workspaceName="qa-agent-codex"`, `timeout=60`. | Four Codex worktree panes open in the new workspace with the 2x2 layout from ST-Q-MCPA-132. | IPC response, `list_panes` declaredAgent fields, screenshot or layout dump, and worktree paths. | PASS only if all panes report `declaredAgent=codex`, layout is 2x2, workspace is closed, and created `qa-agent-codex-*` worktrees are cleaned. |
| ST-Q-MCPA-136 | Claude Code panes get the same even batch layout. | Same as ST-Q-MCPA-135 with `agent="claude"`, names `qa-agent-claude-a`..`d`, and `workspaceName="qa-agent-claude"`. | Four Claude Code panes open in 2x2. | IPC response, `list_panes`, screenshot or layout dump, and worktree paths. | PASS only if all panes report `declaredAgent=claude`, layout is 2x2, workspace/worktrees are cleaned. |
| ST-Q-MCPA-137 | OpenCode panes get the same even batch layout. | Same as ST-Q-MCPA-135 with `agent="opencode"`, names `qa-agent-opencode-a`..`d`, and `workspaceName="qa-agent-opencode"`. | Four OpenCode panes open in 2x2. | IPC response, `list_panes`, screenshot or layout dump, and worktree paths. | PASS only if all panes report `declaredAgent=opencode`, layout is 2x2, workspace/worktrees are cleaned. |
...
never accepted as proof of the launched executable. Agent-facing prompts must
not mention MCP servers, integrations, function names, or tool names; a guard
in the runner rejects such prompts before any pane is created.

| ID | Case | Driver | Expected |
| --- | --- | --- | --- |
| ST-Q-MCPA-154 | Ask a real Codex to open OpenCode in an exact directory and exchange a reply. | `QA/scripts/soyeht_agent_driven_e2e.py` against Soyeht Dev; prompt states only the user's intent. | Without being told a function or integration, Codex chooses the current MCP; the child process is observed as `opencode --auto` in `QA`; request and reply both persist with MCP contract 3; parent observes the reply; child never becomes active. |
| ST-Q-MCPA-155 | Ask a real OpenCode to open Claude in an exact directory and exchange a reply. | Same behavioral runner and implementation-blind prompt. | Without a tool hint, OpenCode opens the child observed as `claude` in `docs`; both durable messages carry contract 3; parent completes after the reply; child never becomes active. |
| ST-Q-MCPA-156 | Ask a real Claude to open Codex in an exact directory and exchange a reply. | Same behavioral runner and implementation-blind prompt. | Without a tool hint, Claude opens the child observed as `codex --yolo` in `TerminalApp`; both durable messages carry contract 3; parent completes after the reply; child never becomes active. |
| ST-Q-MCPA-157 | A Release MCP process inherits the Dev automation directory and attempts a mutation. | Direct call through the `soyeht` Release integration to Soyeht Dev | Dev rejects the mutation when `mcpClientContractVersion` is missing or not 3, or when `mcpClientProfile` is not `dev`; it creates/changes/sends nothing. Read-only discovery remains available. |
...
| ST-Q-MCPA-160 | A real recipient agent has unfinished user input when a different real agent relays an MCP v2 message. | `QA/scripts/soyeht_agent_driven_e2e.py --typing-collision-only` against Soyeht Dev; both setup and follow-up state only user intent. | The ring Codex → OpenCode, OpenCode → Claude, and Claude → Codex runs with each recipient focused and holding text without Enter. In all three cases the durable `deferredTerminal` timestamp stays empty and the relay token stays absent before Enter; after Enter the message is delivered, the recipient replies through MCP contract 3, and the sender observes the reply. Dynamic alternate-screen capture of the draft itself is informative but not used as the gate oracle. |
| ST-Q-MCPA-161 | Build Soyeht Dev from a git worktree with a persistent signing identity. | `scripts/build-install-soyeht-dev --build-only` | The script discovers the primary checkout's ignored `Local.xcconfig`, verifies `com.soyeht.mac.dev`, passes strict deep signature validation, and fails closed if `TeamIdentifier` is empty or `not set`. |
| ST-Q-MCPA-162 | Register the signed Soyeht Dev TCC identity without requiring pre-existing UI automation permission. | `scripts/build-install-soyeht-dev --request-permissions`, then inspect macOS Privacy & Security. | Debug launch argument invokes the productized Agent Permissions flow; `Soyeht Dev` appears separately from `Soyeht` in Accessibility, the user can enable it, and the stable signing requirement survives subsequent signed rebuilds. |
| ST-Q-MCPA-163 | Hold real unfinished human input while each primary CLI receives a message from a different real agent. | `QA/scripts/soyeht_agent_driven_e2e.py --physical-keyboard-only` against the signed, Accessibility-approved Soyeht Dev app. | The ring Codex → OpenCode, OpenCode → Claude, and Claude → Codex uses an actual Quartz mouse click, macOS Accessibility keystrokes, and a physical Return in the exact Soyeht Dev window. For every route, the draft is visibly present, the relay is absent and its `deferredTerminalDeliveredAt` is empty before Return, then the relay is visible and timestamped after Return; the recipient replies through MCP contract 3 / server 2.0.0, the sender observes the reply, and independently observed argv/cwd prove the requested agents and directories. |
| ST-Q-MCPA-164 | Abandon a real unfinished draft without pressing Enter while an MCP v2 relay is queued. | Same physical runner with `--draft-release-action backspace`, `ctrl-u`, or `ctrl-c`, across Codex → OpenCode, OpenCode → Claude, and Claude → Codex. | Deleting every character with physical Backspace releases the relay on all three recipients. The valid instant-cancel shortcuts release it without Return: `Ctrl+U` on Claude/Codex and `Ctrl+C` on OpenCode. Negative control: OpenCode `Ctrl+U` leaves its draft visible, so the timestamp remains empty and the relay remains safely queued. Every released route completes the MCP v2 round trip and independently proves argv/cwd. |

## Execution Reports

- [2026-08-21 Soyeht Dev stable Accessibility identity](../runs/2026-08-21-soyeht-dev-accessibility/report.md)
- [2026-08-21 MCP 2.0 natural-language agent E2E](../runs/mcp2-natural-language-agent-e2e-2026-08-21.md)
